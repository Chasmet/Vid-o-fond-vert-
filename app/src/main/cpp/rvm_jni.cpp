...
class RvmCore {
public:
    bool init(AAssetManager* assets, std::string& error) {
        ...
        if (net.load_param(assets, "rvm_mobilenetv3.ncnn.param") != 0) {
            error = "param RVM introuvable";
            return false;
        }
        if (net.load_model(assets, "rvm_mobilenetv3.ncnn.bin") != 0) {
            error = "poids RVM introuvables";
            return false;
        }
        return true;
    }
    ...
    bool predict(JNIEnv* env, jobject bitmap, int targetSize, bool highQuality,
        ...
        if (AndroidBitmap_getInfo(env, bitmap, &info) != ANDROID_BITMAP_RESULT_SUCCESS) {
            error = "bitmap Android illisible";
            return false;
        }
        ...
        if (AndroidBitmap_lockPixels(env, bitmap, &pixels) != ANDROID_BITMAP_RESULT_SUCCESS
                || pixels == nullptr) {
            error = "pixels caméra inaccessibles";
            return false;
        }
        ...
        int wpad = 0;
        int hpad = 0;
        const bool downsample = std::max(w, h) > targetSize;
        ...
        if (inPad.empty() || inSmallPad.empty()) {
            error = "préparation RVM échouée";
            return false;
        }
        ...
        if (ex.input("in0", inPad) != 0 || ex.input("in1", inSmallPad) != 0
                || ex.input("in2", r1) != 0 || ex.input("in3", r2) != 0
                || ex.input("in4", r3) != 0 || ex.input("in5", r4) != 0) {
            error = "entrées RVM invalides";
            clearState();
            return false;
        }
        ...
        if (ex.extract("out7", next1, 1) != 0
            ...
            error = "mémoire temporelle RVM invalide";
            clearState();
            return false;
        }
        r1 = next1;
...
Java_com_chasmet_fondvertstudio_RvmNcnnEngine_nativeCreate(
    ...
    if (!mgr) {
        throwJava(env, "AssetManager indisponible");
        return 0L;
    }
    RvmCore* engine = new RvmCore();
    std::string error;
    if (!engine->init(mgr, error)) {
        delete engine;
